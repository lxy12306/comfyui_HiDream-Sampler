...
from .hi_diffusers.common.uitls import *
...
def load_models(model_type, use_uncensored_llm, base_model_dir = ""):
    if not hidream_classes_loaded:
    ...
    if base_model_dir != "":
        llama_model_name = load_hg_model(llama_model_name, base_model_dir, "HiDream")
    else :
    ...
    if base_model_dir != "":
        model_path = load_hg_model(model_path, base_model_dir, "HiDream", )
    else :
...
class HiDreamCacheClean:
    @classmethod
    def INPUT_TYPES(s):  # (Keep method the same)

        return {
            "required": {
                "model_cache": ("HiDreamModelCache"),
            }
        }
    ...
    def cleanup_models(self, model_cache):
        print(model_cache)
        if model_cache:
...
class HiDreamSampler:
    ...
    RETURN_TYPES = ("IMAGE", "HiDreamModelCache")
    RETURN_NAMES = ("image", "model_cache")
    FUNCTION = "generate"
    CATEGORY = "HiDream"
    @staticmethod
    def cleanup():
        if HiDreamSampler._model_cache:
            clean = HiDreamCacheClean()
            clean.cleanup_models(HiDreamSampler._model_cache)

    def cleanup_models(self):
        if self._model_cache and not any(key.startswith(f"HiDreamSampler_{self._model_type}_") for key in self._model_cache.keys()):
                print(f"Clearing cache because a different model is cached before loading {self._model_type}...")
                clean = HiDreamCacheClean()
                clean.cleanup_models(self._model_cache)

    def generate(self, base_model_dir, model_type, prompt, fixed_resolution, seed, override_steps, override_cfg, override_width, override_height, **kwargs):
        ...
        if pipe is None:
            ...
            try:
                pipe, config = load_models(model_type, use_uncensored_llm, base_model_dir)
...
class HiDreamSamplerAdvanced:
    ...
    def generate(self, base_model_dir, model_type, primary_prompt, negative_prompt, width, height, seed, scheduler,
...
class HiDreamImg2Img:
    ...
    def cleanup_models(self):
        if self._model_cache:
                print(f"Clearing img2img cache before loading {self._model_type}...")
                clean = HiDreamCacheClean()
                clean.cleanup_models(self._model_cache)

    def preprocess_image(self, base_model_dir, image, target_height=None, target_width=None):
    ...
    def generate(self, base_model_dir, model_type, image, denoising_strength, prompt, negative_prompt,
        ...
        if pipe is None:
            ...
            try:
                ...
                txt2img_pipe, config = load_models(model_type, use_uncensored_llm, base_model_dir)
...
NODE_CLASS_MAPPINGS = {
    ...
    "HiDreamCacheClean": HiDreamCacheClean,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "HiDreamCacheClean": "HiDream Cache Clean",
}
